...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.groupBy(c => c).map(l => (l._1, l._2.length)).toList
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.sortBy(_._2).map(tuple => new Leaf(tuple._1, tuple._2))
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    val b1 : CodeTree = trees.head
    val b2 : CodeTree = trees.tail.head
    val fork: Fork = new Fork(b1, b2, chars(b1) ::: chars(b2), weight(b1) + weight(b2))
